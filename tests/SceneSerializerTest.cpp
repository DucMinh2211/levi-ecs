...
namespace {
    bool verify(flecs::world& world) {
        ...
        return position && position->x == 10.0f && position->y == 20.0f
            && sprite && sprite->texturePath == "assets/quoted\"name.png"
            && collider && collider->radius == 12.5f
            && camera && camera->zoom == 2.0f && camera->active
            && camera->maxShakeOffset == 20.0f && camera->shakeRemaining == 0.0f
            && script && std::get<int>(script->values.at("hp")) == 80
            && std::get<bool>(script->values.at("active"))
            && unnamedPositions == 1;
    }
}
