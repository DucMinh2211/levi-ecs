        ...
        world.system<const Position2D, const Sprite2D>("RenderSprites")
            ...
            .each([](flecs::entity entity, const Position2D& position, const Sprite2D& sprite) {
                ...
                SDL_FRect destination = {
                    screenPosition.x - pivot.x, screenPosition.y - pivot.y, width, height
                };
                if (texture) {
                    SDL_RenderTextureRotated(renderer->ptr, texture, nullptr, &destination,
                        static_cast<double>(angle), &pivot, SDL_FLIP_NONE);
                } else {
