...
namespace Levi {
    namespace {
        ...
        bool isSceneEntity(flecs::entity entity) {
            ...
            if (!name.empty() && (name[0] == '$' || name == "World" || name == "Query" || name == "Observer")) return false;
        ...
        private:
            ...
            void whitespace() { while (position_ < source_.size() && std::isspace(static_cast<unsigned char>(source_[position_]))) ++position_; }
            bool take(char expected) { whitespace(); if (position_ < source_.size() && source_[position_] == expected) { ++position_; return true; } return false; }
            void expect(char expected) { if (!take(expected)) fail(std::string("expected '") + expected + "'"); }
            void literal(const char* text) { const auto size = std::strlen(text); if (source_.compare(position_, size, text) != 0) fail("invalid literal"); position_ += size; }
            [[noreturn]] void fail(const std::string& message) const { throw std::runtime_error(message + " at byte " + std::to_string(position_)); }
        ...
        std::string sceneToJson(const Scene& scene) {
            ...
            for (std::size_t i = 0; i < scene.size(); ++i) {
                ...
                out << "    {\"parent\":" << entity.parent << ",\"name\":\"" << escapeJson(entity.name) << "\",\"components\":{";
                bool first = true;
                auto key = [&](const char* name) { if (!first) out << ','; first = false; out << '"' << name << "\":"; };
                if (entity.position) { key("Position2D"); writeVector(out, *entity.position); }
                if (entity.scale) { key("Scale2D"); writeVector(out, *entity.scale); }
                if (entity.rotation) { key("Rotation2D"); out << '[' << entity.rotation->angle << ','; writeVector(out, entity.rotation->pivot); out << ',' << static_cast<int>(entity.rotation->pivotType) << ']'; }
                if (entity.sprite) { key("Sprite2D"); out << "{\"texture\":\"" << escapeJson(entity.sprite->texturePath) << "\",\"size\":"; writeVector(out, entity.sprite->size); out << '}'; }
                if (entity.aabb) { key("AABBCollider2D"); out << "{\"size\":"; writeVector(out, entity.aabb->size); out << ",\"offset\":"; writeVector(out, entity.aabb->offset); out << '}'; }
                if (entity.circle) { key("CircleCollider2D"); out << "{\"radius\":" << entity.circle->radius << ",\"offset\":"; writeVector(out, entity.circle->offset); out << '}'; }
                if (entity.camera) {
                    key("Camera2D");
                ...
                for (std::size_t s = 0; s < entity.scripts.size(); ++s) {
                    ...
                    for (const auto& [name, value] : script.values) {
                        ...
                        std::visit([&out](const auto& item) {
                            ...
                            if constexpr (std::is_same_v<T, float>) out << "\"type\":\"float\",\"value\":" << item;
                            else if constexpr (std::is_same_v<T, int>) out << "\"type\":\"int\",\"value\":" << item;
                            else if constexpr (std::is_same_v<T, bool>) out << "\"type\":\"bool\",\"value\":" << (item ? "true" : "false");
                            else out << "\"type\":\"string\",\"value\":\"" << escapeJson(item) << '"';
                        }, value);
                        out << '}';
                    }
                    out << "}}";
        ...
        Scene sceneFromJson(const std::string& source) {
            ...
            for (const auto& entityJson : root.at("entities").array()) {
                ...
                if (auto it = components.find("Position2D"); it != components.end()) { auto v = readVector(it->second); entity.position = Position2D{v.x, v.y}; }
                if (auto it = components.find("Scale2D"); it != components.end()) { auto v = readVector(it->second); entity.scale = Scale2D{v.x, v.y}; }
                if (auto it = components.find("Rotation2D"); it != components.end()) {
                    const auto& a = it->second.array();
                    entity.rotation = Rotation2D{static_cast<float>(a.at(0).number()), readVector(a.at(1)), static_cast<PivotType>(static_cast<int>(a.at(2).number()))};
                }
                ...
                if (auto it = components.find("AABBCollider2D"); it != components.end()) {
                    entity.aabb = AABBCollider2D{readVector(it->second.at("size")), readVector(it->second.at("offset"))};
                }
                if (auto it = components.find("CircleCollider2D"); it != components.end()) {
                    entity.circle = CircleCollider2D{static_cast<float>(it->second.at("radius").number()), readVector(it->second.at("offset"))};
                }
                ...
                for (const auto& scriptJson : entityJson.at("scripts").array()) {
                    ...
                    for (const auto& [name, field] : scriptJson.at("values").object()) {
                        ...
                        if (type == "float") script.values[name] = static_cast<float>(field.at("value").number());
                        else if (type == "int") script.values[name] = static_cast<int>(field.at("value").number());
                        else if (type == "bool") script.values[name] = field.at("value").boolean();
                        else if (type == "string") script.values[name] = field.at("value").string();
                        else throw std::runtime_error("unsupported script value type: " + type);
                    }
        ...
        template<typename T>
        void binaryWrite(std::vector<unsigned char>& out, const T& value) {
        ...
        std::vector<unsigned char> sceneToBinary(const Scene& scene) {
            ...
            for (const auto& entity : scene) {
                ...
                if (entity.sprite) { binaryWriteString(out, entity.sprite->texturePath); binaryWrite(out, entity.sprite->size); }
                ...
                for (const auto& script : entity.scripts) {
                    ...
                    for (const auto& [name, value] : script.values) {
                        ...
                        std::visit([&out](const auto& item) {
                            using T = std::decay_t<decltype(item)>;
                            if constexpr (std::is_same_v<T, std::string>) binaryWriteString(out, item);
                            else binaryWrite(out, item);
                        }, value);
                    }
                }
            }
        ...
        Scene sceneFromBinary(const std::vector<unsigned char>& data) {
            if (data.size() < 8 || std::memcmp(data.data(), "LEVISCN1", 8) != 0) throw std::runtime_error("invalid binary scene header");
                ...
                for (std::uint32_t s = 0; s < scriptCount; ++s) {
                    ...
                    for (std::uint32_t v = 0; v < valueCount; ++v) {
                        ...
                        switch (reader.read<std::uint8_t>()) {
                            case 0: script.values[name] = reader.read<float>(); break;
                            case 1: script.values[name] = reader.read<int>(); break;
                            case 2: script.values[name] = reader.string(); break;
                            case 3: script.values[name] = reader.read<bool>(); break;
                            default: throw std::runtime_error("invalid binary script value");
                        }
                    }
                    entity.scripts.push_back(std::move(script));
                }
        ...
        bool setError(std::string* error, const std::string& value) { if (error) *error = value; return false; }
    }

    std::string SceneSerializer::toJson(flecs::world& world) { return sceneToJson(captureScene(world)); }

    bool SceneSerializer::fromJson(flecs::world& world, const std::string& json, std::string* error) {
        try { restoreScene(world, sceneFromJson(json)); return true; }
        catch (const std::exception& exception) { return setError(error, exception.what()); }
    }
    ...
    std::vector<unsigned char> SceneSerializer::toBinary(flecs::world& world) { return sceneToBinary(captureScene(world)); }

    bool SceneSerializer::fromBinary(flecs::world& world, const std::vector<unsigned char>& data, std::string* error) {
        try { restoreScene(world, sceneFromBinary(data)); return true; }
        catch (const std::exception& exception) { return setError(error, exception.what()); }
    }
